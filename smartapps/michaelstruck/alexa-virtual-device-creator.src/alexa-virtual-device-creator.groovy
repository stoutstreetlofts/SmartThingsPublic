...
preferences {
    ...
    page name:"pageSrchDevices"
}
def mainPage() {
    dynamicPage(name: "mainPage", uninstall: false, install: true) {
        section("New device information"){
            input "addSwitchName", "text", title: "Device Label", description: "Enter a unique label name for the virtual device", required: true, submitOnChange:true
...
// Show "pageSrchDevices" page
def pageSrchDevices() {
	dynamicPage(name: "pageSrchDevices", title: "Search for Devices", install: false, uninstall: false) {
    	def repsonse
        if (getChildDevices().find{it.label == addSwitchName}){
            repsonse="There is already a device labled '${addSwitchName}'.\n\nTap < to go back and change the device label name."
        }
...
//Common modules
def getSwitchAbout(){ return "Device creation based on Alexa Virtual Device Creator"+
							 "created and Copyright © 2018 Michael Struck"+
                             "modified for limited use by Stout Street Lofts Project"+
                             "\n\n"+
                             "https://github.com/MichaelStruck/SmartThingsPublic"
                             }
